...
-- competitive-verifier: PROBLEM https://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=DSL_2_G&lang=ja
main :: IO ()
main = do
  (!n, !q) <- ints2
  ...
  res <- (`VU.mapMaybeM` qs) $ \case
    (0, !l, !r, !x) -> do
      LSeg.applyIn seg (l - 1) r $ RangeAdd (Sum x)
      pure Nothing
    (1, !l, !r, !_) -> do
      Sum x <- LSeg.prod seg (l - 1) r
      pure $ Just x
    _ -> error "unreachable"
