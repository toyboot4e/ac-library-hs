...
module AtCoder.Internal.GrowVec (GrowVec (..), new, read, pushBack, deleteBack, length, unsafeFreeze, freeze) where
...
-- | \(O(1)\).
deleteBack :: (HasCallStack, PrimMonad m, VU.Unbox a) => GrowVec (PrimState m) a -> m ()
deleteBack GrowVec {..} = do
