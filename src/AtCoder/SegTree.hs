...
set self@SegTree {..} p x = do
  let !_ =
        runtimeAssert (0 <= p && p < nSt) $
          "set: index out of bounds (`" ++ show p ++ "` over the number of vertices `" ++ show nSt ++ "`)"
...
get SegTree {..} p = do
  let !_ =
        runtimeAssert (0 <= p && p < nSt) $
          "get: index out of bounds (`" ++ show p ++ "` over the number of vertices `" ++ show nSt ++ "`)"
...
prod SegTree {..} l0 r0 = inner (l0 + sizeSt) (r0 + sizeSt - 1) mempty mempty
  where
    !_ =
      runtimeAssert (0 <= l0 && l0 <= r0 && r0 <= nSt) $
        "prod: invalid range `[" ++ show l0 ++ ", " ++ show r0 ++ ")` over the number of vertices `" ++ show nSt ++ "`"
...
maxRight SegTree {..} l0 f
  ...
  where
    !_ =
      runtimeAssert (0 <= l0 && l0 <= nSt) $
        "maxRight: left index out of bounds (`" ++ show l0 ++ "`) over the number of vertices `" ++ show nSt ++ "`"
    !_ = runtimeAssert (f mempty) "maxRight: `f mempty` returned `False`"
    inner l !sm = do
      let l' = chooseBit l
...
minLeft SegTree {..} r0 f
  ...
  where
    !_ =
      runtimeAssert (0 <= r0 && r0 <= nSt) $
        "minLeft: right index out of bounds (`" ++ show r0 ++ "`) over the number of vertices `" ++ show nSt ++ "`"
    !_ = runtimeAssert (f mempty) "minLeft: `f empty` returned `False`"
    inner r !sm = do
      let r' = chooseBit $ r - 1
...
update :: (HasCallStack, PrimMonad m, Monoid a, VU.Unbox a) => SegTree (PrimState m) a -> Int -> m ()
update SegTree {..} k = do
