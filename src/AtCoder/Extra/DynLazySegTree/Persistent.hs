...
-- \([l_0, r_0)\). The nodes are instantinated as needed and the required capacity is
-- /approximately/ \(8q \log L\), where \(q\) is the number of mutable operations and \(L\) is the
...
-- | \(O(n)\) Creates a `DynLazySegTree` of capacity \(n\) for interval \([l_0, r_~)\) with initial
...
-- | \(O(\log L)\) Resets an interval \([l, r)\) to `initialProdLdst`.
--
-- ==== Constraints
-- - \(l_0 \le l \le r \le r_0\)
--
