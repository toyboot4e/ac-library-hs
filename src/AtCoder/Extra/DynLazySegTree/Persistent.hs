...
-- \([l_0, r_0)\). Is is dynamic in that the nodes are instantinated as needed. The required
-- capacity is approximately \(4q \log L\), where \(q\) is the number of mutable operations and
...
--
-- >>> let capacityFor n q = 8 * q * max 2 (ceiling (logBase 2 (fromIntegral n) :: Double))
-- >>> let len = 4; q = 3
