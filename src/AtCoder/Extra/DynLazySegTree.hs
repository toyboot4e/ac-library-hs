...
-- The nodes are instantinated as needed and the required capacity is /approximately/ \(4q \log L\),
...
-- | \(O(n)\) Creates a `DynLazySegTree` of capacity \(n\) for interval \([l_, r_0)\) with `mempty`
