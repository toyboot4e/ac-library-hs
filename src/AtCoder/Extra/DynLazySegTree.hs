...
-- Is is dynamic in that the nodes are instantinated as needed. The required capacity is
-- approximately \(2q \log L\), where \(q\) is the number of mutable operations and \(L\) is the
...
--
-- >>> let capacityFor n q = 4 * q * max 2 (ceiling (logBase 2 (fromIntegral n) :: Double))
-- >>> let len = 4; q = 3
-- >>> seg <- Seg.new @_ @(Affine1 Int) @(Sum Int) (capacityFor len q) 0 4
--
...
-- | \(O(n)\) Creates a `DynLazySegTree` of capacity \(n\) for interval \([l, r)\) with `mempty` as
-- initial leaf values.
--
-- @since 1.2.1.0
{-# INLINE new #-}
...
-- | \(O(n)\) Creates a `DynLazySegTree` of capacity \(n\) for interval \([l, r)\) with initial
