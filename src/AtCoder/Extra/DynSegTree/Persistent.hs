...
-- | A dynamic, persistent segment tree that covers a half-open interval \([l_0, r_0)\). The nodes
-- are instantinated as needed and the required capacity is /approximately/ \(4q \log L\), where
...
--
-- >>> let capacityFor len q = 2 * q * max 2 (2 + ceiling (logBase 2 (fromIntegral len) :: Double))
-- >>> let len = 4; q = 2
...
recommendedCapacity n q = 2 * q * max 2 (2 + ceiling (logBase 2 (fromIntegral n) :: Double))
...
-- | \(O(\log L)\) Resets an interval \([l, r)\) to `mempty`.
--
-- ==== Constraints
-- - \(l_0 \le l \le r \le r_0\)
--
