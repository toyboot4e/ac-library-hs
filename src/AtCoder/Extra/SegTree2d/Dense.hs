...
-- | \(O(hw)\) Creates a `DenseSegTree2d` for \([0, w) \times [0, h)\).
--
-- @since 1.2.3.0
{-# INLINEABLE new #-}
new ::
...
modifyM DenseSegTree2d {..} f x0_ y0_ = do
  ...
  stToPrim $ do
    -- FIXME: why ignoring zero? I think zero is required?:
    --           1  2  3
    -- 11  6  5  1  2  3
...
prod seg x1 x2 y1 y2 = stToPrim $ prodST seg x1 x2 y1 y2
...
prodST seg@DenseSegTree2d {..} x1 x2 y1 y2 = do
  let !_ = ACIA.checkRect "AtCoder.Extra.SegTree2d.Dense.prodST" x1 x2 y1 y2 wDst hDst
