...
-- @since 1.2.2.0
{-# INLINE maxRight #-}
maxRight :: (HasCallStack, PrimMonad m, Num a, VU.Unbox a) => FenwickTree (PrimState m) a -> (a -> Bool) -> m Int
maxRight ft f = maxRightM ft (pure . f)
...
maxRightM :: forall m a. (HasCallStack, PrimMonad m, Num a, VU.Unbox a) => FenwickTree (PrimState m) a -> (a -> m Bool) -> m Int
maxRightM FenwickTree {..} f = do
  b0 <- f 0
...
-- @since 1.2.2.0
{-# INLINE minLeft #-}
minLeft :: (HasCallStack, PrimMonad m, Num a, VU.Unbox a) => FenwickTree (PrimState m) a -> (a -> Bool) -> m Int
minLeft ft f = minLeftM ft (pure . f)
...
-- @since 1.2.2.0
{-# INLINEABLE minLeftM #-}
minLeftM :: forall m a. (HasCallStack, PrimMonad m, Num a, VU.Unbox a) => FenwickTree (PrimState m) a -> (a -> m Bool) -> m Int
minLeftM FenwickTree {..} f = do
  b0 <- f 0
