...
prop_maxRight :: QC.NonNegative Int -> QC.NonEmptyList (QC.NonNegative Int) -> QC.Property
prop_maxRight (QC.NonNegative xRef) (QC.NonEmpty xs_) =
...
prop_minLeft :: QC.NonNegative Int -> QC.NonEmptyList (QC.NonNegative Int) -> QC.Property
prop_minLeft (QC.NonNegative xRef) (QC.NonEmpty xs_) =
  let xs = VU.fromList $ map (\(QC.NonNegative x) -> x) xs_
      expected = (VU.length xs -) . VU.length . VU.takeWhile (<= xRef) $ VU.scanl1' (+) $ VU.reverse xs
      res = runST $ do
        ft <- FT.build xs
        FT.minLeft ft (<= xRef)
   in expected QC.=== res
