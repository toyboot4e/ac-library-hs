...
module Tests.Extra.DynSegTree.Persistent where
...
capacityFor n q = 2 * q * max 2 (ceiling (logBase 2 (fromIntegral n) :: Double))
...
instance QC.Arbitrary Init where
  arbitrary = do
    ...
    pure $ Init n q (VUM.generate n (\i -> f (i + l0) (i + l0 + 1))) (Seg.buildWith cap l0 (l0 + n) f)
    where
      f l r = Sum . modInt $ sum [l .. r - 1]
...
-- | ac-library-hs.
handleAcl :: (HasCallStack) => Seg.DynSegTree RealWorld (Sum Mint) -> VUM.MVector RealWorld Seg.Index -> Query -> IO Result
